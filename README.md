...
`마나 위버스` 세계관에서 마왕 **보이드**가 **마나 코어**를 파괴해 세상에서 마법이 사라지고(= **마나 침묵**), 영웅들은 스킬 없이 **육탄전**만 하게 됩니다.  
유저는 차원의 틈에서 깨어난 **대정령(Player)** 으로, **마나 파편(퍼즐 블록)** 을 맞춰 에너지를 만들어 영웅들이 다시 스킬을 쓰게 돕는 존재입니다.

- **프롤로그 톤**: 진지한 세계관 + 코믹한 연출(데미지 `-1` 등)로 시작
- **초기 동료**: 테오(전사, 3성, 빨강) → 토비(힐러, 1성, 노랑) → 유리(마법사, 1성, 초록)
- **로드맵**: Chapter 1(파티 결성) → Chapter 2(3인 기본 파티 완성) → Chapter 3(소환 상점 오픈/덱 확장) → 에필로그(마왕 토벌)

자세한 내용은 [`Docs/Scenario.md`](Docs/Scenario.md)를 참고하세요.
...
| [`Docs/GDD/전투 및 퍼즐 로직_1_0.md`](Docs/GDD/전투%20및%20퍼즐%20로직_1_0.md) | (레거시) 단일 히어로 기반 초기 전투/콤보/궁극기 규칙 |
| [`Docs/GDD/히어로 및 스킬 1_0.md`](Docs/GDD/히어로%20및%20스킬%201_0.md) | `HeroData` / `SkillData` JSON 스키마, 스킬 ActionType·TargetScope·상태이상 |
| [`Docs/GDD/스킬 시스템 고도화 1_0.md`](Docs/GDD/%EC%8A%A4%ED%82%AC%20%EC%8B%9C%EC%8A%A4%ED%85%9C%20%EA%B3%A0%EB%8F%84%ED%99%94%201_0.md) | Actions[] + Target(Team/Strategy) 기반 모듈식 스킬 구조 및 타겟팅 규약 |
| [`Docs/GDD/히어로 직업 및 고유 스킬 1_0.md`](Docs/GDD/%ED%9E%88%EC%96%B4%EB%A1%9C%20%EC%A7%81%EC%97%85%20%EB%B0%8F%20%EA%B3%A0%EC%9C%A0%20%EC%8A%A4%ED%82%AC%201_0.md) | 직업(Class), 등급별 스킬 트리거(1~3성), 고유 스킬 블록/궁극기 컷인 |
